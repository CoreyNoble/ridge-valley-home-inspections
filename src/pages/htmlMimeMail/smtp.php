...
class smtp
{
    ...
    public function connect($params = array())
    {
        if (!isset($this) || !isset($this->status)) {
            ...
            if (is_resource($this->connection)) {
            	// try an ehlo first, then helo if it fails
            	if($this->ehlo() || $this->helo()){
    ...
    */
    public function send($params = array())
    {
        ...
        if ($this->is_connected()) {
            ...
            $result = (substr(trim($serverResponse=$this->get_data()), 0, 3) === '250');
            //$this->rset();
            if(!$result){
                $this->errors[] = 'SEND command failed: '.$serverResponse;
            }
            return $result;
        } else {
    ...
    */
    private function helo()
    {
        if(is_resource($this->connection)
            ...
            $this->errors[] = 'HELO command failed, output: ' . trim($error);
            return false;
        }
    }
    ...
    */
    private function ehlo()
    {
        if (is_resource($this->connection)
            ...
            $this->errors[] = 'EHLO command failed, output: ' . trim($error);
            return false;
        }
    }
    ...
    */
    private function rset()
    {
        if (is_resource($this->connection)
            ...
            $this->errors[] = 'RSET command failed, output: ' . trim($error);
            return false;
        }
    }
    ...
    */
    private function quit()
    {
        if(is_resource($this->connection)
            ...
            $this->errors[] = 'QUIT command failed, output: ' . trim($error);
            return false;
        }
    }
    ...
    */
    private function mail($from)
    {
        if ($this->is_connected()
            ...
            $this->errors[] = 'FROM command failed: '.trim($error);
            return false;
        }
    }
    ...
    */
    ...
    {
        if($this->is_connected()
            AND $this->send_data('RCPT TO:<'.$to.'>'.(($this->delivery_receipt)?' NOTIFY=SUCCESS,FAILURE,DELAY ORCPT=rfc822;'.trim($to):''))
