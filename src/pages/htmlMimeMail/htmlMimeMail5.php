...
class htmlMimeMail5
{
    ...
    */
    ...
    {
        preg_match_all('/(\w*[\x80-\xFF]+\w*\s?)/', $input, $matches);
        ...
        switch ($type) {
            case 'mail':
                ...
                if (empty($this->headers['Date'])) {
                    // if no date header was passed, add one in before sending
                    $headers[] = 'Date: ' . $this->encodeHeader(date('D, d M y H:i:s O'), $this->build_params['head_charset']);
                }
            ...
            case 'smtp':
                ...
                $send_params['delivery_receipt'] = $this->delivery_receipt;
